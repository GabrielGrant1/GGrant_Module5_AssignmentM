...
public class FirstNameUtility {
    ...
    public int encodeFirstName(String firstName, String middleInitial) throws MissingNameException {
        int firstNameEncoding = 0;
        if(firstName.equals("")) {
            throw new MissingNameException("First Name");
        }else{
            String firstInit = Character.toString(firstName.charAt(0));
            if(middleInitial.equals("")){
                if(firstNameCodeMap.containsKey(firstName)){
                    firstNameEncoding = firstNameCodeMap.get(firstName);
                }else{
                    firstNameEncoding = firstInitialCodeMap.get(firstInit);
                }
            }else if(firstNameCodeMap.containsKey(firstName)){
                firstNameEncoding = firstNameCodeMap.get(firstName) + middleInitialCodeMap.get(middleInitial);
            }else{
                firstNameEncoding = firstInitialCodeMap.get(firstInit) + middleInitialCodeMap.get(middleInitial);
            }
        }
        return firstNameEncoding;
    }
}
